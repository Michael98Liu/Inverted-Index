...
void writetoDisk(map < string, vector<unsigned int>> postingList, map<string, tuple<long,int>> &dic) {
	ofstream pList("D:\\mystuff\\InvertedIndex\\postinglists\\001", ofstream::binary);
...
vector<int> query(string query, map<string, tuple<long, int>> &dic) {
	...
	for (list<string>::iterator it = stream.begin(); it != stream.end(); ++it) {
		...
		if (dicvoc != dic.end()) {
			...
			char memblock[64] = { 0 };
			ifstream file("D:\\mystuff\\InvertedIndex\\postinglists\\001", ifstream::binary);
			if (file) {
...
int main() {
	...
	DIR *dir;
	struct dirent *ent;
	if ((dir = opendir("D:\\mystuff\\InvertedIndex\\files")) != NULL) {
